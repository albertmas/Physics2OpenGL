...
bool ModulePhysics3D::CleanUp()
{
	...
	btCollisionShape* iterator; // = colShapeList.getFirst()->data;
	for (int i = 0; i < colShapeList.count(); i++)
	{
		colShapeList.at(i, iterator);
		delete iterator;
		iterator = nullptr;
	}
	colShapeList.clear();
	...
	PhysBody3D* temp;
	for (int j = 0; j < pbodyList.count(); j++)
	{
		pbodyList.at(j, temp);
		delete temp;
		temp = nullptr;
